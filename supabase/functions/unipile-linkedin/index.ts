...
serve(async (req) => {
  ...
  try {
    // For webhook requests, verify the secret
    ...
    if (webhookSecret === WEBHOOK_SECRET) {
      ...
      if (webhookData.event === 'message_received') {
        console.log('Processing received message:', webhookData.message_id);
        
        // Store the message in the database
        const { error: insertError } = await supabaseAdmin
          .from('linkedin_messages')
          .insert({
            unipile_message_id: webhookData.message_id,
            sender_name: webhookData.sender.attendee_name,
            sender_profile_url: webhookData.sender.attendee_profile_url,
            content: webhookData.message,
            thread_id: webhookData.chat_id,
            received_at: webhookData.timestamp,
            is_outbound: false,
      ...
      // Handle existing code for manual requests
    } else {
